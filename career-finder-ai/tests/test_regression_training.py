"""
test_regression_training.py – Lightweight tests for rubric-assisted regression training.
"""
...
def test_precision_at_k_between_zero_and_one():
    frame = pd.DataFrame(
        [
            {"profile_id": "p1", "predicted_score": 95, "target_score": 80},
            {"profile_id": "p1", "predicted_score": 90, "target_score": 75},
            {"profile_id": "p1", "predicted_score": 50, "target_score": 40},
            {"profile_id": "p1", "predicted_score": 40, "target_score": 30},
            {"profile_id": "p1", "predicted_score": 30, "target_score": 20},
            {"profile_id": "p2", "predicted_score": 60, "target_score": 50},
        ]
    )
...
def test_model_output_paths_are_under_repo_models():
    for path in (RIDGE_MODEL_FILE, RANDOM_FOREST_MODEL_FILE, GRADIENT_BOOSTING_MODEL_FILE):
        assert path.name.startswith("rubric_assisted_"), (
            f"Expected rubric_assisted_* model name, got: {path.name}"
        )
        assert path.parent.name == "models"

    assert VECTORIZER_FILE.name == "rubric_assisted_vectorizer.joblib"
    assert METRICS_FILE.parent.name == "processed"
    assert PREDICTIONS_FILE.name == "rubric_assisted_regression_predictions.csv"


def test_metrics_file_name():
    assert METRICS_FILE.name == "rubric_assisted_regression_model_metrics.csv"


def test_rubric_model_track_label():
    """The TRACK_LABEL must be 'rubric_assisted' to avoid confusion."""
    from app.train_regression_model import TRACK_LABEL
    assert TRACK_LABEL == "rubric_assisted"
